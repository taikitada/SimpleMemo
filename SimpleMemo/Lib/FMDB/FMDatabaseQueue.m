...
#endif
        if (!success) {
            LOG(@"FMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(void (^)(FMDatabase *db))block {
    ...
    dispatch_sync(_queue, ^() {
        ...
        if ([db hasOpenResultSets]) {
            LOG(@"Warning: there is at least one open result set around after performing [FMDatabaseQueue inDatabase:]");
