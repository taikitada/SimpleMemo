...
- (int)busyRetryTimeout {
    ...
    LOG(@"FMDB: busyRetryTimeout no longer works, please use retryTimeout");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    LOG(@"%s:%d", __FUNCTION__, __LINE__);
    LOG(@"FMDB: setBusyRetryTimeout does nothing, please use setRetryTimeout:");
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        LOG(@"The FMDatabase %@ is not open.", self);
        ...
        while (idx < queryCount) {
            ...
            if (_traceExecution) {
                if ([obj isKindOfClass:[NSData class]]) {
                    LOG(@"data: %ld bytes", (unsigned long)[(NSData*)obj length]);
                }
                else {
                    LOG(@"obj: %@", obj);
                }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (idx != queryCount) {
        LOG(@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            LOG(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            LOG(@"DB Query: %@", sql);
        }
    }
        ...
        // uh oh.
        if (_logsErrors) {
            LOG(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            LOG(@"DB Query: %@", sql);
        }
    }
    else {
        // wtf?
        if (_logsErrors) {
            LOG(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            LOG(@"DB Query: %@", sql);
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            LOG(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            LOG(@"DB Query: %@", sql);
        }
    }
